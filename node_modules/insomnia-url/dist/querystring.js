...
const joinUrlAndQueryString = (url, qs) => {
    ...
    const baseUrl = base || '';
    const joiner = exports.getJoiner(base);
...
strict) => {
    ...
    if (!strict || param.value) {
        ...
        const value = exports.flexibleEncodeComponent(param.value || '').replace(/%2C/gi, ',');
        const name = exports.flexibleEncodeComponent(param.name || '');
        return `${name}=${value}`;
    }
    else {
        return exports.flexibleEncodeComponent(param.name);
    }
};
...
strict) => {
    ...
    for (const param of parameters) {
        const built = exports.buildQueryParameter(param, strict);
        if (!built) {
            continue;
        }
        items.push(built);
...
const smartEncodeUrl = (url, encode) => {
    ...
    const urlWithProto = protocol_1.setDefaultProtocol(url);
    if (!encode) {
        return urlWithProto;
    }
    else {
        // Parse the URL into components
        const parsedUrl = url_1.parse(urlWithProto);
        // ~~~~~~~~~~~ //
        // 1. Pathname //
        // ~~~~~~~~~~~ //
        if (parsedUrl.pathname) {
            ...
            parsedUrl.pathname = segments
                .map(s => exports.flexibleEncodeComponent(s, URL_PATH_CHARACTER_WHITELIST))
                .join('/');
        }
        // ~~~~~~~~~~~~~~ //
        // 2. Querystring //
        // ~~~~~~~~~~~~~~ //
        if (parsedUrl.query) {
            const qsParams = exports.deconstructQueryStringToParams(parsedUrl.query);
            const encodedQsParams = [];
            for (const { name, value } of qsParams) {
                encodedQsParams.push({
                    name: exports.flexibleEncodeComponent(name),
                    value: exports.flexibleEncodeComponent(value),
                });
            }
            parsedUrl.query = exports.buildQueryStringFromParams(encodedQsParams);
